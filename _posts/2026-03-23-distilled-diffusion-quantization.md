...
最近在量化文生图模型时，我碰到一个很反我直觉的现象：在相近的量化配置下，蒸馏模型比 `base model` 精度下降更不模型。
...
### 1. 蒸馏学到的不是“所有可能的解”，而是 teacher 的稳定映射

蒸馏时，`student` 学的首先不是“整个数据分布里所有可能的解”，而是 `teacher` 已经给出来的一种输出映射。
...
“蒸馏模型更好量化”应该被理解成一个部署层面的现象：它不一定说明蒸馏模型更简单，却很可能说明它们在目标采样路径上更规整、更少犯错，也更不容易把量化噪声一步步放大。
