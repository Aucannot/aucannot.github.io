...
蒸馏模型把 timestep 压缩了，同一个 prompt 换 seed 出来的图都差不多。但奇怪的是 FID 反而比 base 好——不同 prompt 能出不同的图，只是同一个 prompt 下出不了不同的图。论文问的就是：为什么？
...
也就是说，**蒸馏模型里有这些多样性的"开关"，它在自由生成的时候就是不按**。
...
这篇论文的思路很清晰：先排除"学没学到"的问题，再用可视化定位"什么时候出了问题"，最后一步验证就解决了问题。
